...
class GameClass:
    ...
    def __init__(self, num_player: int = 0):
    ...
    def create_player_class(self):
        ...
        for x in range(self.num_player):
    ...
    def initialize(self):
        ...
        if self.user_decision() == True: #bot players' actions are all check in this round by default
    ...
    def reset_whole_game(self):
        ...
        self.reset_player_bet()


    def reset_player_bet(self):
        """Reset each player's bet."""
            ...
            try:
                ...
                if self.decision == "f" or self.decision == "F":
                    ...
                    if self.decision == "b" or self.decision == "B":
                        ...
                        if self.user_bet <= 0 or self.user_bet > (self.user_class.money - self.user_class.total_bet):
                            raise ValueError
                        else:
                            self.user_class.total_bet += self.user_bet
    ...
    def display_action(self):
        ...
        for player in self.round_player_class[:-1]:  # except user, which is the last index
            if player.rank > self.threshold:
                print(f"{player.name}: fold")
                self.round_player_class.remove(player)
            elif player.rank == self.threshold or (player.money - player.total_bet) == 0:
                print(f"{player.name}: check")
            else:
                self.remain = player.money - player.total_bet
                if self.remain >= self.bet:
                    print(f"{player.name}: bet ${self.bet}")
                    player.total_bet += self.bet
                    self.money_pool += self.bet
                else:
                    print(f"{player.name}: bet ${self.remain}")
                    self.money_pool += self.remain
    ...
    def final_summary(self):
        ...
        for player in self.round_player_class:
            ...
            if player in self.winner_class:
                player.money += int(self.money_pool / len(self.winner_class))


    def display_final_result(self):
        """Display final result."""
        for player in self.all_player_class:
            print(f"{player.name} {','.join(player.own_card)} ${player.money}")
